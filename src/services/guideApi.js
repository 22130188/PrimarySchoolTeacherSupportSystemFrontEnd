...
async function parse(response) {
  ...
  if (!response.ok) throw new Error(typeof body === 'string' ? body : body?.message || `Yêu cầu thất bại (${response.status})`);
  return body;
}
...
export async function saveGuide(payload) {
  ...
  return parse(await fetch(url, { method: payload.id ? 'PUT' : 'POST', headers: authHeaders(), body: JSON.stringify(payload) }));
}

export async function deleteGuide(id) {
  return parse(await fetch(`${BASE}/api/admin/guides/${id}`, { method: 'DELETE', headers: authHeaders() }));
}

export async function reorderGuides(ids) {
  return parse(await fetch(`${BASE}/api/admin/guides/reorder`, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify({ ids }) }));
}
...
export async function uploadGuideImage(file) {
  ...
  if (!response.ok || !data.secure_url) throw new Error(data?.error?.message || 'Không tải được ảnh lên Cloudinary');
