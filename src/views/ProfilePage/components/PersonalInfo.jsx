...
export default function PersonalInfo({ user, onUpdate }) {
  ...
  const initialFullName = user?.fullName || user?.name || user?.username || '';
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div>
          ...
          {roleId === 1 ? (
            <input type="text" value={getDisplayPosition()} readOnly className={`${inputCls} bg-gray-50 text-gray-500 cursor-not-allowed`} />
          ) : roleId === 2 ? (
            <div className="relative">
      ...
      <div className="flex justify-between mt-8">
        <button
          ...
          onClick={() => setForm({
            ...
            dateOfBirth: user?.dateOfBirth || '',
            gender: user?.gender || '',
            position: roleId === 2 ? (user?.position || 'Giáo viên') : 'Giáo viên',
            email: user?.email || '',
            phone: user?.phone || '',
          })}
