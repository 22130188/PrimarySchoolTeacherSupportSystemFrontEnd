        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <div className="flex max-h-[80vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
              <div>
                <p className="mb-2 inline-flex rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">Audio của tôi</p>
                <h3 className="text-xl font-bold text-slate-900">Chọn âm thanh từ thư viện</h3>
                <p className="mt-1 text-sm text-slate-500">Các audio bạn đã lưu, không bao gồm audio của người dùng khác.</p>
              </div>
              <button type="button" onClick={() => setShowAudioLibrary(false)} className="rounded-full p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="Đóng thư viện audio">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="min-h-0 flex-1 p-6">
              {loadingAudioLibrary ? (
                <div className="flex min-h-44 items-center justify-center"><Loader className="h-7 w-7 animate-spin text-blue-600" /></div>
                ...
                <div className="max-h-[52vh] space-y-3 overflow-y-auto pr-1">
                  {libraryAudios.map((audio) => {
                    ...
                    const isSelected = formData.audioUrl === audio.audioUrl;
                    return (
                      <article key={audio.id || audio.audioUrl} className={`rounded-2xl border p-4 ${isSelected ? 'border-blue-400 bg-blue-50 ring-2 ring-blue-100' : 'border-slate-200 bg-white'}`}>
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div className="min-w-0">
                            <p className="truncate font-semibold text-slate-900" title={audioName}>{audioName}</p>
                          ...
                          <button type="button" onClick={() => handleSelectLibraryAudio(audio.audioUrl)} className="shrink-0 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
                ...
                <div className="flex min-h-44 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-center">
                  <p className="font-semibold text-slate-700">Bạn chưa có audio trong thư viện</p>
                  <p className="mt-1 text-sm text-slate-500">Hãy lưu audio tại công cụ chuyển văn bản thành giọng nói trước.</p>
                </div>
              )}
            </div>
          </div>
